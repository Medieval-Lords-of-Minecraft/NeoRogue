...
public class Overload extends Equipment implements Power {
	...
	public void setupReforges() {
		addReforge(WeaponEnchantmentElectrified.get(), WeaponEnchantmentElectrified2.get());
	}
	...
	public void setupItem() {
		item = createItem(Material.GLOWSTONE_DUST,
				GlossaryTag.PASSIVE.tag(this) + " " + GlossaryTag.POWER.tag(this) + ". Activates after applying " + GlossaryTag.ELECTRIFIED.tag(this) + " " + DescUtil.val(5) + " times. On kill, deal " + GlossaryTag.LIGHTNING.tag(this, damage) + " damage + the number of "
				+ GlossaryTag.ELECTRIFIED.tag(this) + " stacks the killed enemy has multiplied by " + DescUtil.val(mult) + " in an area.");
	}
}
