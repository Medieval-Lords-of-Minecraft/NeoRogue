...
public class Gambit extends Equipment {
	...
	private static final TriggerCondition cond = (p, pdata, in) -> {
		if (!pdata.hasStatus(StatusType.FOCUS)) {
			Util.msgRaw(p, "You need Focus to use this ability!");
			return false;
		}
		return true;
	};
	...
	public void setupItem() {
		item = createItem(Material.DRAGON_HEAD,
				"Requires " + GlossaryTag.FOCUS.tag(this) + ". On cast, lose " + GlossaryTag.FOCUS.tag(this, 1) + ". If you kill " + DescUtil.val(2) + " " +
				"enemies within " + DescUtil.val("3s") + ", permanently increase your damage by " + DescUtil.val(damage) + ".");
	}
}
