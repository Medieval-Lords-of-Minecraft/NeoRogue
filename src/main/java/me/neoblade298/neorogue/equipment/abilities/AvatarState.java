...
public class AvatarState extends Equipment implements Power {
	...
	item = createItem(Material.GLOWSTONE,
			GlossaryTag.PASSIVE.tag(this) + " " + GlossaryTag.POWER.tag(this) + ". Activates after casting " + DescUtil.val(ACTIVATION_THRES)
					+ " abilities while above 50% mana. Increase mana regen by " + DescUtil.val(mreg) + ", health regen by "
					+ DescUtil.val(hreg) + ", and gain " + GlossaryTag.SHIELDS.tag(this, shields)
					+ ", all lasting 10s.");
	}
}
