...
public class Twilight extends Equipment implements Power {
	...
	public void setupItem() {
		item = createItem(Material.ECHO_SHARD,
				GlossaryTag.PASSIVE.tag(this) + " " + GlossaryTag.POWER.tag(this) + ". Activates after receiving " + GlossaryTag.STEALTH.tag(this) + " once while above " + DescUtil.val("50%") + " mana and stamina. Whenever you receive " + GlossaryTag.STEALTH.tag(this) + ", increase its duration by " + DescUtil.val(duration) + ", " +
				"gain " + GlossaryTag.EVADE.tag(this, evade) + " [<white>8s</white>], and " + DescUtil.val(8) + " stamina.");
	}
}
