...
public class BlessedEdge extends Equipment {
	...
	public BlessedEdge(boolean isUpgraded) {
		super(ID, "Blessed Edge", isUpgraded, Rarity.UNCOMMON, EquipmentClass.WARRIOR,
				EquipmentType.ABILITY, EquipmentProperties.ofUsable(15, 15, 6, 0));
		damage = 100;
	...
	public void initialize(PlayerFightData data, Trigger bind, EquipSlot es, int slot, SessionEquipment sessionEq) {
		data.addTrigger(id, bind, new EquipmentInstance(data, sessionEq, slot, es, (pdata, inputs) -> {
			...
			data.addTrigger(id, Trigger.PRE_BASIC_ATTACK, (pdata2, in) -> {
				...
				FightInstance.getFightData(target.getUniqueId()).applyStatus(StatusType.SANCTIFIED, data, sanct, -1, this);
	...
	public void setupItem() {
		item = createItem(Material.GLOWSTONE_DUST,
				"On cast, your next basic attack deals " + DescUtil.val(damage) + " " + GlossaryTag.LIGHT.tag(this) + " damage and applies " + DescUtil.val(sanct) +
