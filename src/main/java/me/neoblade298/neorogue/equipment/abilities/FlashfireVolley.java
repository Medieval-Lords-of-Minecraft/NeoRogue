...
public class FlashfireVolley extends Equipment {
	...
	public void setupItem() {
		item = createItem(Material.FIRE_CHARGE,
				GlossaryTag.PASSIVE.tag(this) + ". Upon firing a bow straight down, cancel the basic attack and launch yourself upward, dealing " +
				DescUtil.val(bluntDamage) + " " + GlossaryTag.BLUNT.tag(this) + " damage to nearby enemies. " +
				"Then fire " + DescUtil.val(3) + " arrows in quick succession that deal " + DescUtil.val(fireDamage) + " " +
				GlossaryTag.FIRE.tag(this) + " damage. If these arrows hit an enemy damaged by the launch, " +
				"apply " + GlossaryTag.BURN.tag(this, burn) + ".");
	}
}
