...
public class SadisticNature extends Equipment {
	...
	public void setupItem() {
		item = createItem(Material.SPECTRAL_ARROW,
				"After " + GlossaryTag.CHARGE.tag(this) + " for " + DescUtil.val((CHARGE_TIME / 20) + "s") + ", fire a projectile that deals " +
				...
				"given " + DescUtil.val("Slowness 3") + ", and dealt an additional " + 
				DescUtil.val(bonusDamage) + " damage. " +
				"Additionally, spawn a " + GlossaryTag.TRAP.tag(this) + " [<white>5s</white>] on the block that deals " +
				DescUtil.val(trapDamage) + " damage and increases your " + GlossaryTag.TRAP.tag(this) + 
				" damage by " + DescUtil.val((int)(TRAP_DAMAGE_INC * 100) + "%") + " when it's set off.");
	}
}
