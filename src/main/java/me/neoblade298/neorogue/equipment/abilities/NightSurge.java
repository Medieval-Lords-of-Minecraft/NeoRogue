...
public class NightSurge extends Equipment implements Power {
	...
	public void setupItem() {
		item = createItem(Material.ECHO_SHARD,
				GlossaryTag.PASSIVE.tag(this) + " " + GlossaryTag.POWER.tag(this) + ". Activates after applying " + GlossaryTag.INSANITY.tag(this) + " " + DescUtil.val(3) + " times and dealing " + GlossaryTag.DARK.tag(this) + " damage " + DescUtil.val(3) + " times. Dealing " + GlossaryTag.DARK.tag(this) + " damage to an enemy with " +
				GlossaryTag.INSANITY.tag(this) + " spawns a projectile from them that fires " +
				"opposite the direction they're facing, dealing " + GlossaryTag.DARK.tag(this, damage) + 
				" damage. The target hit also takes this damage. " + DescUtil.val("2s") + " cooldown per enemy.");
	}
}
