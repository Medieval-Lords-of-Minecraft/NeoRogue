...
public class Veilpiercer extends Equipment {
	...
	public void initialize(PlayerFightData data, Trigger bind, EquipSlot es, int slot, SessionEquipment sessionEq) {
		...
		data.addTrigger(id, Trigger.APPLY_STATUS, (pdata, in) -> {
			...
			if (ev.isStatus(StatusType.STEALTH) && ev.getTarget() == data && ev.getStacks() > 0) resetIfCurrent(data);
			return TriggerResult.keep();
		});
	...
	private void resetIfCurrent(PlayerFightData data) {
		if (data.wasLastBasicAttackSetBy(EquipSlot.HOTBAR, this)) {
