...
public class GrendelsStaff extends Equipment {
	...
	public GrendelsStaff(boolean isUpgraded) {
		super(
				...
				EquipmentProperties.ofWand(isUpgraded ? 55 : 45, 0.9, 0, 1, RANGE, DamageType.DARK, Sound.ENTITY_PLAYER_ATTACK_SWEEP)
		);
	...
	public void initialize(PlayerFightData data, Trigger bind, EquipSlot es, int slot, SessionEquipment sessionEq) {
		...
		ProjectileGroup proj = new ProjectileGroup(new GrendelsStaffProjectile(data, this, slot, attackCounter));
	...
	private class GrendelsStaffProjectile extends Projectile {
		...
		public GrendelsStaffProjectile(PlayerFightData data, GrendelsStaff eq, int slot, ActionMeta attackCounter) {
			super(2.0, RANGE, 1);
			this.size(0.6, 0.6);
			this.data = data;
			this.eq = eq;
		...
		public void onHit(FightData hit, Barrier hitBarrier, DamageMeta meta, ProjectileInstance proj) {
			...
			if (count % intellectFreq == 0) {
				data.applyStatus(StatusType.INTELLECT, data, 1, -1, GrendelsStaff.this);
			}
