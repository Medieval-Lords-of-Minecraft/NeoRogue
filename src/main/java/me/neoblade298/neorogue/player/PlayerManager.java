...
public class PlayerManager implements IOComponent {
	...
	public PlayerManager(boolean lightweight) {
		...
		try (Connection con = NeoCore.getConnection("NeoRogue-PlayerManager");
			...
			stmt.execute("CREATE TABLE IF NOT EXISTS neorogue_run_results (id BIGINT NOT NULL AUTO_INCREMENT, uuid VARCHAR(36) NOT NULL, ts BIGINT NOT NULL, playerClass VARCHAR(40) NOT NULL, notoriety INT NOT NULL, partySize INT NOT NULL DEFAULT 1, exp INT NOT NULL DEFAULT 0, playtime BIGINT NOT NULL DEFAULT 0, won TINYINT NOT NULL, competitive TINYINT NOT NULL DEFAULT 0, PRIMARY KEY (id));");
			try {
