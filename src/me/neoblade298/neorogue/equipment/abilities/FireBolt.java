...
public class FireBolt extends Equipment {
	...
	private static final ParticleContainer lightning = new ParticleContainer(Particle.FIREWORK).count(3).spread(0.3, 0.3),
		fire = new ParticleContainer(Particle.FLAME).count(3).spread(0.3, 0.3);
	...
	public FireBolt(boolean isUpgraded) {
		super(
				ID , "Fire Bolt", isUpgraded, Rarity.UNCOMMON, EquipmentClass.MAGE, EquipmentType.ABILITY,
	...
	private void fire(Player p, PlayerFightData data, boolean isLightning) {
		...
		if (isLightning) Sounds.firework.play(p, p);
		else Sounds.fire.play(p, p);
		for (LivingEntity ent : TargetHelper.getEntitiesInLine(p, start, end, tp)) {
			FightInstance.dealDamage(new DamageMeta(data, damage, isLightning ? DamageType.LIGHTNING : DamageType.FIRE), ent);
		}
	}
