...
public abstract class FightInstance extends Instance {
	...
	public static FightData getFightData(UUID uuid) {
		if (!fightData.containsKey(uuid)) {
			...
			if (am == null) {
				Bukkit.getLogger().info("Failed to create new FightData for " + ent.getName() + " " + ent.getType() + " " + ent.getUniqueId());
				return null;
			}
	...
	public void start() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				double mapPieceBonus = map.getPieces().size() * map.getPieces().size() / 6.0;
				double chunkBonus = sumChunkCount / 4.0;

				double toActivate = 2.0 + rngBonus + mapPieceBonus + chunkBonus;
