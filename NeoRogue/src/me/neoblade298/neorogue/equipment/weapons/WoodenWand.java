...
public class WoodenWand extends Equipment {
	...
	public void initialize(Player p, PlayerFightData data, Trigger bind, EquipSlot es, int slot) {
		ProjectileSettings settings = new WoodenWandProjectile(p);
		data.addSlotBasedTrigger(id, slot, Trigger.LEFT_CLICK, new EquipmentInstance(p, this, slot, (d, inputs) -> {
			weaponSwing(p, data);
			settings.start(data.getInstance(), data);
			return TriggerResult.keep();
		}));
	}
	
	private class WoodenWandProjectile extends ProjectileSettings {
		private Player p;
